...
项目在 M1 Pro 上记录到的端到端基准为：10 分钟音频 `14.97 秒`，RTF `0.0249`，
约 40 倍实时；按相同负载线性折算，1 小时音频约 `90 秒`。实际耗时受音频长度、
说话人数、录音质量、机器温度、后台负载和 macOS 版本影响。

在清晰普通话、领域匹配的项目样本中，人工抽查的文字正确率多数可达 90% 以上。
该数字不是公开测试集上的统一 CER/WER，也不是对所有口音、噪声、重叠说话或专业
术语场景的保证。Gemini 精修可以改善断句、标点和表达，但不应被当作原始识别准确率，
重要内容仍应人工复核。
